module physpkg
  ...
  character(16) macrop_scheme
  character(16) microp_scheme
...
contains
  ...
  subroutine phys_inidat(cam_out, pbuf2d)
    ...
    character(11), parameter :: sub = 'phys_inidat'
    integer tpert_idx, qpert_idx, pblh_idx

    logical :: found, found2=.false.
    integer ierr
    ...
    if (.not. cam_grid_check(grid_id)) then
      call endrun(trim(sub)//': Internal error, no "physgrid" grid')
    end if
    ...
    if (associated(fh_topo) .and. .not. aqua_planet) then
      call infld('SGH', fh_topo, dim1name, dim2name, 1, pcols, begchunk, endchunk, tptr, found, gridname='physgrid')
      ...
      call infld('SGH30', fh_topo, dim1name, dim2name, 1, pcols, begchunk, endchunk, tptr_2, found, gridname='physgrid')
      if (found) then
      ...
      call infld('LANDM_COSLAT', fh_topo, dim1name, dim2name, 1, pcols, begchunk, endchunk, tptr, found, gridname='physgrid')
      if (.not.found) call endrun(' ERROR: LANDM_COSLAT not found on topo dataset.')

      call pbuf_set_field(pbuf2d, landm_idx, tptr)
    else
      call pbuf_set_field(pbuf2d, sgh_idx  , 0.0_r8)
      call pbuf_set_field(pbuf2d, sgh30_idx, 0.0_r8)
      call pbuf_set_field(pbuf2d, landm_idx, 0.0_r8)
    end if
  ...
  subroutine phys_init(phys_state, phys_tend, pbuf2d, cam_in, cam_out)
    ...
    else if (microp_scheme == 'SPCAM_m2005') then
      call conv_water_init()
    end if
...
!$OMP PARALLEL DO PRIVATE (c, phys_buffer_chunk)
    do c = begchunk, endchunk
      ...
      call tphysbc(ztodt, phys_state(c), phys_tend(c), phys_buffer_chunk, cam_out(c), cam_in(c))
    end do
...
!$OMP PARALLEL DO PRIVATE (c, NCOL, phys_buffer_chunk)
  ...
  subroutine tphysac(ztodt, cam_in, cam_out, state, tend, pbuf)
    ...
    if (chem_is_active()) then
      call chem_timestep_tend(state, ptend, cam_in, cam_out, ztodt, pbuf, fh2o=fh2o)
      call physics_update(state, ptend, ztodt, tend)
    ...
    ! --------------------------------------------------------------------------
    call vertical_diffusion_tend(ztodt ,state , cam_in, surfric, obklen, ptend, ast, pbuf)
    ...
    call check_energy_chng(state, tend, 'gwdrag', nstep, ztodt, zero, zero, zero, flx_heat)
    ...
    if (aqua_planet) then
      labort = .false.
      do i = 1, ncol
        if (cam_in%ocnfrac(i) /= 1.0_r8) then
          labort = .true.
    ...
    if (microp_scheme == 'RK') then
      ...
      do macmic_it = 1, cld_macmic_num_steps
        ...
        if (macrop_scheme /= 'CLUBB_SGS') then
          ...
          ! the full time (ztodt).
          call physics_ptend_scale(ptend, 1.0_r8 / cld_macmic_num_steps, ncol)
          call physics_update(state, ptend, ztodt, tend)
        ...
        if (use_subcol_microp) then
          ...
          !        physics_ptend_scale.
          if (trim(subcol_scheme) == 'SILHS') &
            call subcol_SILHS_fill_holes_conserv(state, cld_macmic_ztodt, ptend, pbuf)
          ...
          call massless_droplet_destroyer(cld_macmic_ztodt, state, ptend)
