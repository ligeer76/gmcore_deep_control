module polar_avg
  ...
  public polar_average ! support for LR dycore polar averaging
...
contains
  ...
  subroutine polar_average2d(field)
    ...
    call shr_reprosum_calc(n_pole, sum(1:1), ln, plon, 1, gbl_count=plon, commid=mpicom)
    call shr_reprosum_calc(s_pole, sum(2:2), ls, plon, 1, gbl_count=plon, commid=mpicom)
  ...
  subroutine polar_average3d(nlev, field)
    ...
    call shr_reprosum_calc(n_pole, sum(:,1), ln, plon, nlev, gbl_count=plon, commid=mpicom)
    call shr_reprosum_calc(s_pole, sum(:,2), ls, plon, nlev, gbl_count=plon, commid=mpicom)
