...
contains
  ...
  subroutine cam_physics_add_output(tag, dtype)
    ...
    call fiona_add_var(tag, 'precl'  , long_name='Large-scale precipitation'  , units='m s-1', dim_names=dims, dtype=dtype)
    call fiona_add_var(tag, 'precc'  , long_name='Convective precipitation'   , units='m s-1', dim_names=dims, dtype=dtype)
    call fiona_add_var(tag, 'olr'    , long_name='Outgoing longwave Radiation', units='w m-2', dim_names=dims, dtype=dtype)
  ...
  subroutine cam_physics_output(tag, iblk)
    ...
    call fiona_output(tag, 'precc'  , reshape(state%precc    , mesh%cell_count_2d(1:2)), start=mesh%cell_start_2d, count=mesh%cell_count_2d)
    call fiona_output(tag, 'olr'    , reshape(state%olr      , mesh%cell_count_2d(1:2)), start=mesh%cell_start_2d, count=mesh%cell_count_2d)
